...
class PromptBuilder:
    ...
    def build(
        ...
        *,
        ...
        """Merge prompt sections while respecting the character budget."""
        ...
        # Preserve the system instructions, runtime evidence, documents,
        # and current request when the prompt must be shortened.
        priority_sections: list[str] = []

        if system_prompt:
            priority_sections.append(f"SYSTEM INSTRUCTIONS (trusted):\n{system_prompt}")

        if context.tool_results:
            priority_sections.append(
                f"TOOL OUTPUTS (untrusted external data):\n{self._format_list(context.tool_results)}"
            )

        if context.documents:
            priority_sections.append(f"DOCUMENTS (untrusted retrieved data):\n{self._format_list(context.documents)}")

        priority_sections.append(f"USER REQUEST:\n{context.request.text}")
